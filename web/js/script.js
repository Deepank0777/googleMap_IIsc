...
function drawAll(error, ageCSV, idCSV, occupations) {
	...
	var colorBar = d3.scale.ordinal()
		.domain(["Resources-fossil fuels","Ecosystems Quality-Acidification and Eutrophication","Human Health-Climate Change",
			"Resources-minerals","Human health-Ozone layer depletion","Human Health-Respiratory effects caused by organic substances",
			"Human health-Ionising radiation","Human Health-Respiratory effects caused by inorganic substances",
			"Human Health-Carcinogenics","Ecosystems Quality-Ecotoxicity"])
		.range(["#DC143C", "#7FFF00", "#00FFFF", "#991C71", "#4F54A8", "#07997E", "#9A7D0A","#0000FF","	#8B4513","#006400"]);	
	...
	var elementsPerBar = 10,
		barChartHeight = 0.8,
		barChartHeightOffset = 0.16;
		...
		for (var i = 0; i < nodeCount; i++) {
			...
			if(node.ID in dataById) {
				...
				if(node.ID.lastIndexOf(currentID, 0) === 0  & !hidden) {
					...
					var fontSizeLabels = Math.round(nodeR / 28);
					...
					if (Math.round(barHeight) > 1) {
						...
						for (var j = 0; j < bars.length; j++) {
							...
							chosenContext.fillRect(nodeX + -nodeR*0.1 , bar.barPiecePosition, bar.width+1, barHeight-5);
							...
							if(drawLabelText & showText) {
								...
								chosenContext.textBaseline = "middle"; 
                                                                var s=new String(bar.age);
                                                                if(s.length<35){
                                                                    chosenContext.fillText(s, nodeX + -nodeR*0.1, bar.barPiecePosition+0.5*barHeight);
                                                                }
                                                                else{
                                                                    chosenContext.fillText(s.substr(0,36), nodeX + -nodeR*0.1, bar.barPiecePosition+0.3*barHeight);
                                                                    chosenContext.fillText(s.substr(36,s.length), nodeX + -nodeR*0.1, bar.barPiecePosition+0.7*barHeight);
                                                                }
							...
							if(drawValueText & showText) {
								...
								chosenContext.textBaseline = "right"; 
		...
		if (interpolator) {
			...
			if (timeElapsed >= duration) {
				...
				d3.select(".legendWrapper").selectAll(".legendText")
					.text(function(d) { console.log(commaFormat(Math.round(scaleFactor * d * d / 10)*10)) ;return commaFormat(Math.round(scaleFactor * d * d / 10)*10); });
...
function genColor(){
	var ret = [];
	// via http://stackoverflow.com/a/158041111111183
	if(nextCol < 16777215){
...
function createLegend(scaleFactor) {
	...
	//Place the value next to the line
		...
		.attr('dy', '0.3em')
		.text(function(d) { return commaFormat(Math.round(scaleFactor * d * d / 10)*100); });
